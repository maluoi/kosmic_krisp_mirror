...
for cmd in curl 7z; do
    if ! command -v "$cmd" &>/dev/null; then
        ...
        echo "Install with: sudo apt install p7zip-full curl"
        exit 1
    fi
done
...
echo "==> Downloading Vulkan SDK metadata..."
...
if [[ -z "$INSTALLER_DAT" ]]; then
    echo "Error: Could not find installer.dat"
    exit 1
fi
...
if [[ -z "$REPO_URL" ]]; then
    echo "Error: Could not find repository URL in installer"
    exit 1
fi
echo "    Found repository: $REPO_URL"

# Fetch Updates.xml to get component details
echo "==> Fetching component manifest..."
...
if [[ -z "$KOSMIC_VERSION" || -z "$KOSMIC_ARCHIVE" ]]; then
    echo "Error: Could not find KosmicKrisp component in manifest"
    exit 1
fi
